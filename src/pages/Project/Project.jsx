 ...
 * @return {number[]} An array with two integers.
 */
function findSumPair(numbers, k) {
    // Create an array of objects containing the number and its index
    let indexedNumbers = numbers.map((num, index) => ({ num, index }));

    // Sort the array based on the number value
    indexedNumbers.sort((a, b) => a.num - b.num);

    // Use two pointers to find the pair that sums to k
    let left = 0;
    let right = numbers.length - 1;

    while (left < right) {
        let sum = indexedNumbers[left].num + indexedNumbers[right].num;
...
// Test cases
let numbers1 = [1, 5, 8, 1, 2];
let k1 = 13;
console.log(findSumPair(numbers1, k1));  // Expected output: [1, 2]

let numbers2 = [111, 35, 222, 20, 50];
let k2 = 70;
console.log(findSumPair(numbers2, k2));  // Expected output: [3, 4]
